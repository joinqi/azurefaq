...
>虚拟机的公网虚拟IP是理论上固定。公网虚拟IP是和云服务DNS关联，而虚拟机是和云服务DNS关联，所以虚拟机，关闭，启动和重启,或者在此云服务下重建，只要对应的云服务没有关闭，且在该云服务下总有一台虚拟机开启着，公网虚拟 IP 就不会变.  
...
>并非所有微软产品的license都可以在VM进行部署的，有的license只可以在物理服务器上进行部署，具体情况请查阅产品license文档，或者咨询相应的产品团队。尽量使用SQL Database代替SQL Server，这样可以充分享受到Windows Azure PaaS所带来的好处，包括成本、扩展性，伸缩性。
...
>Windows Azure可用性集是用来保证构建在Windows Azure虚拟机上的应用程序高可用性的一种机制。可以通过将虚拟机添加到可用性集中来获得这种能力。可用性集的概念与故障域和更新域直接相关。故障域在Windows Azure中是用来避免发生单一失效点（例如网络切换或者共用同一电源模块的服务器机架等）的逻辑定义。事实上，一个故障域非常接近于在同一个机架上的一组服务器。当多台虚拟机被添加到云服务时，可用性集会将这些机器添加到不同的的故障域中。下图展示了具有两个可用性集，每个集合中包含两台机器。
![](/images/Availabilityset.jpg)
